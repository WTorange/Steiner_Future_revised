...
class get_essential_contract(ConnectDatabase):
    '''
    根据期货品种，从wind的CCOMMODITYFUTURESEODPRICES, CINDEXFUTURESEODPRICES, CBONDFUTURESEODPRICES表格中获取日频数据，
    根据持仓量和交易量、到期日期获取前三个主力合约，和最近月合约。分品种输出每天的四类合约数据到\\samba1.quantchina.pro\quanyi4g\temporary\Steiner\data_wash\linux_so\py311\essentialcontract文件夹
    需要from connect_wind import ConnectDatabase，读取wind数据
    目前是根据\\samba1.quantchina.pro\quanyi4g\temporary\Steiner\data_wash\linux_so\py311\品种列表.csv 获取
    '''
...
if __name__ == '__main__':
    symbol_df = pd.read_csv(r'\\samba-1.quantchina.pro\quanyi4g\temporary\Steiner\data_wash\linux_so\py311\品种列表.csv')
